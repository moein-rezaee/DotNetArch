...
Selecting **Cache** or **Message Broker** scaffolds ready‑to‑use Redis or RabbitMQ wrappers. Their NuGet packages are added automatically, interfaces live in `Application/Common/Interfaces` and implementations in `Infrastructure/Services/<Provider>`. The Redis wrapper supports optional Base64 encode/decode and expiry on stored values, while the RabbitMQ wrapper provides publish and subscribe helpers. These services are registered as singletons; `.env.development`, `.env.test`, and `.env.production` files (with uppercase keys for sensitive credentials) and `appsettings.Development.json`, `appsettings.Test.json`, and `appsettings.Production.json` (with PascalCase keys such as host and port) are created in the API project, and the startup project loads them via **DotNetEnv**.
