...
Selecting **Cache** or **Message Broker** scaffolds ready‑to‑use Redis or RabbitMQ wrappers. Their NuGet packages are added automatically, interfaces live in `Application/Common/Interfaces` and implementations in `Infrastructure/Services/<Provider>`. The Redis wrapper supports optional encode/decode using RSA public/private keys and expiry on stored values, while the RabbitMQ wrapper provides publish and subscribe helpers. These services are registered as singletons and rely on `.env.development`, `.env.test`, and `.env.production` files (with uppercase keys like `REDIS__USER` and generated RSA keys) alongside `appsettings.development.json`, `appsettings.test.json`, and `appsettings.production.json` (with PascalCase keys such as Host and Port); the startup project loads them via **DotNetEnv** and accesses values through `IConfiguration`.
