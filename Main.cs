...
class Program
{
    ...
    static void GenerateSolution(string solutionName, string outputPath, string startupProject, string apiStyle)
    {
        ...
        EnsureWritePermissions(solutionDir);
    ...
    public static void EnsureWritePermissions(string path)
    {
        try
        {
            var testFile = Path.Combine(path, ".permtest");
        ...
        catch (UnauthorizedAccessException)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                RunCommand($"icacls \"{path}\" /grant %USERNAME%:F /T", Path.GetPathRoot(path), print: false);
            else
                RunCommand($"chmod -R u+rw \"{path}\"", print: false);
        }
    }
